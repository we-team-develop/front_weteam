...
class MeetingDetail {
  ...
  const MeetingDetail(
      {required this.meetingProject, required this.meetingUserList});

  factory MeetingDetail.fromJson(Map map) {
    List meetingUserList = map['meetingUserList'];
    return MeetingDetail(
        meetingProject: Meeting.fromJson(map),
        meetingUserList: List<MeetingUser>.generate(meetingUserList.length,
...
class MeetingUser {
  ...
  const MeetingUser(
      {required this.id, required this.user, required this.timeList});
  ...
  factory MeetingUser.fromJson(Map map) {
    ...
    return MeetingUser(
        ...
        timeList: List<MeetingTime>.generate(timeSlotList.length,
            (index) => MeetingTime.fromJson(timeSlotList[index])));
  }
}
